...
| Path | Nội dung |
|------|----------|
| `platform-ai/skills/` | Skill markdown |
| `platform-ai/rules/` | Rules `.mdc` |
| `platform-ai/extracts/` | Extract bundles |

Sửa SSOT → `./scripts/platform-ai-link` → mirror sang `.cursor/` + `.kilo/`. Mirror gitignored.
...
- AI infra change is scoped, documented in flow docs, extracts, or `platform-ai/`
- Ran `./scripts/platform-ai-link` after SSOT edits
